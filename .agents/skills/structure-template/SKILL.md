...
6. **Недостающие файлы:** сравнить `find` папки с `find template/<тип>`. Создать только то, чего нет. Уже лежащие файлы не перезаписывать. Лишние относительно шаблона — это нормально; не удалять.
...
9. Лишние относительно шаблона файлы и папки **нормальны** для живой папки. Не удалять без запроса. Если нужен перенос или разнесение текста — остановиться и передать в `structure-revise`.
